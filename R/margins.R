margins.oglmx<-function(object,Vars=NULL,outcomes="All",atmeans=TRUE,AME=FALSE,ascontinuous=FALSE,dummyzero=FALSE,location=NULL){
  ...
  if (is.null(Vars)){
    ...
    # Vars is a character vector with names of variables for which marginal effects are sought.
    betalocs<-match(Vars,attr(object$terms[[1]],"term.labels"))
    if (object$Hetero){
      deltalocs<-match(Vars,attr(object$terms[[2]],"term.labels"))
    } else {
      deltalocs<-NULL
    }
  }
